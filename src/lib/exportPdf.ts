...
export function exportSessionPdf(data: ExportData) {
  ...
  const scoreTextWidth = doc.getTextWidth(score.toFixed(1));

  doc.setFontSize(14);
  setColor(doc, COLORS.textTertiary);
  doc.text('/10', margin + scoreTextWidth + 1, y + 2);
  ...
  doc.setFont('helvetica', 'normal');
  setColor(doc, COLORS.textTertiary);
  doc.text('Generated by Atrium \u2014 AI-Powered Technical Interviews', margin, footY);
  doc.text(new Date().toISOString().split('T')[0], pageW - margin, footY, { align: 'right' });

  // Save
  const filename = `Atrium-Report-${data.candidateName.replace(/\s+/g, '-')}.pdf`;
  doc.save(filename);
}
